...
- For the lens: the MGE provides a flexible, non-parametric description of the lens light that can be cleanly
  subtracted from the data.

- For the source: the MGE captures the extended lensed emission while requiring only **four non-linear free parameters**.
  These parameters control the overall shape of the Gaussian set, and crucially, they are parameterized such that they
  **only govern the source’s size**. This makes the sampling process highly efficient, because the inversion solves for
  all Gaussian intensities linearly, while the non-linear search explores a very low-dimensional space.

The result is that the parameter space can be sampled rapidly and robustly, with a **high probability of finding the
global maximum lens model**. This makes the MGE approach an excellent choice for fast, reliable modeling of strong lens
systems.

Analysis of JWST data has demonstrated that this method not only yields clean lens light subtraction but also provides
a useful first-pass reconstruction of the source light, enabling clear visual inspection of the lensed source emission.

A full description of the MGE method can be found at the following link:
...
def fit(
    ...
    This pipeline fits only the lens galaxy’s light, using a Multi Gaussian Expansion (MGE):  

     - The lens light is represented by 2 sets of 30 Gaussians (60 Gaussians total) [6 non-linear parameters]. 
     
     - Each Gaussian has an `intensity` solved via linear inversion, meaning the MGE introduces 
       ~60 linear parameters but no additional non-linear parameters.  

     - The lens mass model and source light are not included in this stage.  
    """
